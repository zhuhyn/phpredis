...
RedisArray *
ra_make_array(HashTable *hosts, zval *z_fun, zval *z_dist, HashTable *hosts_prev, zend_bool b_index,
              zend_bool b_pconnect, long retry_interval, zend_bool b_lazy_connect, double connect_timeout,
...
#if (PHP_MAJOR_VERSION < 7)
    callable = zend_is_callable_ex(&ra->z_fun, NULL, 0, NULL, NULL, NULL, NULL TSRMLS_CC);
#else
    callable = zend_is_callable_ex(&ra->z_fun, NULL, 0, NULL, NULL, NULL);
#endif
...
ra_call_distributor(RedisArray *ra, const char *key, int key_len, int *pos TSRMLS_DC)
{
    ...
    /* check that we can call the extractor function */
#if (PHP_MAJOR_VERSION < 7)
    callable = zend_is_callable_ex(&ra->z_dist, NULL, 0, NULL, NULL, NULL, NULL TSRMLS_CC);
#else
    callable = zend_is_callable_ex(&ra->z_dist, NULL, 0, NULL, NULL, NULL);
#endif
...
ra_find_key(RedisArray *ra, zval *z_args, const char *cmd, int *key_len) {
    ...
    if (zend_hash_num_elements(Z_ARRVAL_P(z_args)) == 0 ||
        (zp_tmp = zend_hash_index_find(Z_ARRVAL_P(z_args), key_pos)) == NULL || Z_TYPE_P(zp_tmp) != IS_STRING)
    {
        return NULL;
    }
...
ra_move_collection(const char *key, int key_len, zval *z_from, zval *z_to,
                   ...
                   const char **cmd_add, long ttl TSRMLS_DC)
{
